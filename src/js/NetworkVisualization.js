...
export class NetworkVisualization {
  ...
  animateloop() {
    ...
    d3.select("#nodes").select(".target").selectAll("text")
      ...
      .text(n => "Soll: " + n.format(n.target))
      .attr("x", n => n.x + noderadius + 10)
      .attr("y", n => n.y - unit * n.target + 15)
      .attr("opacity", 1)
    ...
    d3.select("#nodes").select(".values").selectAll("text")
      ...
      .attr("x", n => n.x)
      .attr("y", n => n.y + noderadius+10);//- clamp(unit * n.getActivation(), -70 + 40, 70));
    ...
    const nodedrag = d3.drag()
      ...
      .on("drag", function() {
        ...
        if (node.adjustable) {
          ...
          tooltip
            .attr("x", node.x + 20)
            .attr("y", node.y - unit * node.bias + 10)
            .attr("font-size", 25)
            .attr("font-family", "Arial")
            .attr("font-weight", "bold")
            .text(node.bias_txt);//`${node.format(node.bias)}`);
        }
        if (node.constructor.name == "InputNode") {
    ...
    const edgedrag = d3.drag()
      ...
      .on("drag", function() {
        ...
        if (edge.adjustable) {
          ...
          //}
          tooltip
            .attr("x", edge.parameterPosition()[0]+20)
            .attr("y", edge.parameterPosition()[1]+10)
            .attr("font-size", 25)
            .attr("font-family", "Arial")
            .attr("font-weight", "bold")
